...
fun buildJsonSchema(
    ...
    includeSchemaProperty: Boolean = true
): JsonObject {
    ...
    val root = descriptor.createJsonSchema(descriptor.annotations, definitions).toMutableMap()
    if (includeSchemaProperty) {
        val properties = root["properties"] as JsonObject
        root["properties"] =
            JsonObject(properties + mapOf("\$schema" to JsonObject(mapOf("type" to JsonPrimitive("string")))))
    }
